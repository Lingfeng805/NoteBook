...
- **边框**
  ...
  - 指定边框 `border:1px solid #1d649c;`
    - 1px: 边框宽度为1个像素；
    - solid：边框线为实线，也可以是`dashed`(虚线)、`dotted`(点)；
    - 也可指定为无边框`border:none`
...
- **margin、padding**
  ...
  ![image](E:/software/Typora/pictures/20200405184322_921970.png)

  - 指定元素的margin

    ```python
    QTextEdit {
    	margin:10px 11px 12px 13px    # 依次对应上右下左（顺时针）
    }
    ```
...
- **概念**：
  ...
  - **进程**：运行着的程序；(如，开启一个qq程序称为一个进程)
  - **线程**：一个进程中执行的每个任务即为一个线程。系统中每个进程里至少包含一个线程，线程是操作系统创建的，每个线程对应于一个代码执行的数据结构，保存了代码执行过程中的重要的状态信息； 

- 现代计算机的CPU是多核的，每个核都可以执行代码。要运行程序里面的代码，操作系统就会分配一个CPU核心去执行该代码。有的时候，我们希望，能够让更多的CPU核心同时执行我们的程序里面的一些代码。

  - 假如，我们程序里面有个名为 compress 的函数，执行压缩文件的任务。现在有4个大文件，需要压缩。如果是一个CPU核心执行这个函数（单线程的程序），压缩一个文件要10秒钟的话， 那么压缩4个文件，就要40秒。如果我们能够让 4个CPU核心 `同时` 执行压缩函数， 理论上就只要 10秒。

  - 有的时候， 我们有一批任务要执行，而这些任务的执行时间主要耗费在 `非CPU计算` 上面。比如，我们需要到 前程无忧 网站 抓取 python 开发相关的职位信息。我们要抓取几百个网页的内容， 执行这些抓取信息的任务的代码，时间主要耗费在等待网站返回信息上面。 等待信息返回的时候CPU是空闲的。
  ...
  - 如果我们像以前那样 在一个线程里面，用一个循环 依次 获取100个网页的信息，如下：
    ...
    就会有很长的时间耗费在 等待服务器返回信息上面。如果我们能用100个线程，同时运行 获取网页信息的代码， 理论上，可以100倍的减少执行时间。
...
- **Python代码中创建新线程**
  ...
  - 应用程序必须 通过操作系统提供的 **系统调用**，请求操作系统分配一个新的线程。python3 将 系统调用创建线程 的功能封装在 标准库 `threading` 中.
    ...
    # 创建 Thread 类的实例对象
    thread = Thread(    # 当解释器执行到该代码时，创建一个Thread实例对象，target参数指定新线程的入口函数；args参数指定传给入口函数的参数；
